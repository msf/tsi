...
tsi *new_tsi(registry *reg) {
    ...
    mpierr = MPI_Init(NULL, NULL);   /* MPI-2 */
    if (mpierr != MPI_SUCCESS) {
        printf("MPI failed to initialize!\n");
    }
    
    MPI_Comm_size(MPI_COMM_WORLD, &t->n_procs);
    MPI_Comm_rank(MPI_COMM_WORLD, &t->proc_id);
    ...
    printf_dbg("new_tsi(): starting heap\n");
...
int run_tsi(tsi *t) {
    int result;
    int i, j;
    float best_corr, ai_corr, ai_corr_reduce;
    ...
    for (i = 0; i < t->simulations; i++) {
        ...
        if (result) {
            ...
            MPI_Reduce(&ai_corr, &ai_corr_reduce, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
            if (ai_corr_reduce > best_corr) {
                best_corr = ai_corr_reduce;
